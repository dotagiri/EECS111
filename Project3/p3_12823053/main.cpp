...
void fifo_schedule(void)
{
	...
	if(!readyQue.empty()){ //algorithm goes here
		next_t = *readyQue.begin();
		readyQue.erase(readyQue.begin());
		pthread_cond_signal(&cond[next_t]);
	}//fi
	
	//release lock
	pthread_mutex_unlock(&mutex);
}//end of fifo_schedule
...
void edf_schedule(void)
{
	...
	if(!readyQue.empty()){ //algorithm goes here
		//need to iterate through queue to find deadlines
		//use two iterators to compare two deadlines at once
		//use for loop to go through everything in iterator to find if deadline is less than previous
		vector<int>::iterator e_it = readyQue.begin();
		vector<int>::iterator it;

		//tcb = thread control block (global var)
		for(it = readyQue.begin() + 1; it != readyQue.end(); it++){
			if(tcb[*it].deadline < tcb[*e_it].deadline){
				e_it = it;
			}//fi
		}//rof
...
void rm_schedule(void)
{
	//shorter periods get higher priority in RM scheduling
	//shorter time = first to execute
	int next_t;
	//acquire lock
	pthread_mutex_lock(&mutex);
	...
	if(!readyQue.empty()){ //algorithm goes here
		//use two iterators to compare two tasks in queue
		vector<int>::iterator short_it = readyQue.begin();
		vector<int>::iterator it;
		for(it = readyQue.begin() +1; it != readyQue.end(); it++){
			if(tcb[*it].period < tcb[*short_it].period){
				short_it = it;	
			}//fi
		}//rof
		next_t = *short_it;
		readyQue.erase(short_it);
		pthread_cond_signal(&cond[next_t]);
	}//fi
